...
\lstset{
    frame=single,           % Tạo khung đơn giản bao quanh code
    rulecolor=\color{black}, % Màu của khung
    framerule=0.5pt,        % Độ dày của đường kẻ khung
    framesep=3pt,           % Khoảng cách từ khung đến nội dung code
}
...
\fancyfoot[L]{\footnotesize {\ttfamily Extended Report }}
...
\begin{titlepage}
    ...
    % Title and Subtitle
    \textbf{\LARGE{Real-Time Embedded Systems Project}}\\[0.2cm]
    \textbf{\Large{INTERNET OF THINGS}}\\[0.6cm]
    
    % Group Info
    \textbf{\large{CLASS: TN01}}\\[0.2cm]
    \textbf{\large{Lecturer: Lê Trọng Nhân}}
    \begin{table}[h]
...
\end{table}
    ...
    \vspace{4cm} % Adjust space between the table and the advisor section
...
\input{abstract_acknowledgement}
